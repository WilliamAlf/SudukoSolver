...
def remove_possible_value(solver_matrix, square_index, value):
    affected_box = get_box(solver_matrix, square_index)                 # Should sort out solved at this stage
    affected_row = get_row(solver_matrix, square_index)
    affected_column = get_column(solver_matrix, square_index)
    affected_squares = affected_box + affected_row + affected_column    # Contains lots of duplicates

    for solver_square in affected_squares:
        if not solver_square["solved"]:
...
def solve_puzzle(puzzle) -> []:
    ...
    def filling_iteration(unsolved_squares, solver_matrix):
        ...
        for square_index in unsolved_squares:
            solver_square = solver_matrix[square_index]
...
Algorithm Improvements:
    1. Search boxes, columns and rows for possible values that only occurs once within the container
    2. 

Code Improvements :
    1. Make squares objects
"""
